...
// Can change with hasFill
	...
	internal func applyWrapped(
		...
		for index in metrics.tracks.indices {
			...
			switch track.length {
			case .fill(_, _, let maximum, _):
				guard preparedFillActive.indices.contains(index), preparedFillActive[index] else {
					lengths[index] = 0
	...
	private func prepare(
		...
		for (index, element) in metrics.tracks.enumerated() {
			...
			case .uniform(_):
				...
				self.preparedLengths[index] = length
			case .fill(let fraction, _, let maximum, let ifContent):
	...
	private func calculatePreparedSize() -> CGFloat {
		...
		for index in metrics.tracks.indices {
			...
			case .fill:
				visible = preparedFillActive.indices.contains(index) && preparedFillActive[index]
				nonFillLength = 0
			default:
	...
	private func calculateFills(_ available: CGFloat) {
		...
		if layout == .stack {
			for (index, element) in metrics.tracks.enumerated() {
				switch element.length {
				case .fill(let fraction, let minimum, let maximum, _):
					if preparedFillActive.indices.contains(index), preparedFillActive[index] {
	...
	private func allocateFills(
		...
		for (index, element) in metrics.tracks.enumerated() {
			...
			guard case .fill(let fraction, let minimum, let maximum, _) = element.length else {
				continue
			}
